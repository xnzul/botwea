...
const { color, bgcolor } = require('./XNZUL/color')
const { help } = require('./XNZUL/help')
const kagApi = require('@kagchi/kag-api')
const { donasi } = require('./XNZUL/donasi')
const { fetchJson } = require('./XNZUL/fetcher')
const { recognize } = require('./XNZUL/ocr')
const { wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, close } = require('./XNZUL/functions')
const tiktod = require('tiktok-scraper')
const ffmpeg = require('fluent-ffmpeg')
const { removeBackgroundFromImageFile } = require('remove.bg')
const welkom = JSON.parse(fs.readFileSync('./XNZUL/welkom.json'))
const nsfw = JSON.parse(fs.readFileSync('./XNZUL/nsfw.json'))
const samih = JSON.parse(fs.readFileSync('./XNZUL/simi.json'))
...
const config = {
    XNZUL: '👾XNZUL BOT👾', // TOBAT SU ASU
    instagram: 'https://instagram.com/r00t_xnzul', // INFO JANGAN DI UBAH
    nomer: 'wa.me/6283173142772', // INFO SU JNGAN DI UBAH
    youtube: 'https://www.youtube.com/c/XNzulMods', // KINTIL
    whatsapp: 'https://chat.whatsapp.com/D8ViYowjnUXLt7w1VGGuzz', // BABI
...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] QR code is ready, subscribe XNzul Mods`)
})
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝐉𝐚𝐧𝐠𝐚𝐧 𝐛𝐚𝐥𝐢𝐤 𝐥𝐚𝐠𝐢 𝐥𝐨𝐞 𝐚𝐧𝐣 @${num.split('@')[0]}\n 𝐛𝐚𝐥𝐢𝐤 𝐥𝐚𝐠𝐢 𝐠𝐮𝐚 𝐛𝐚𝐧𝐭𝐚𝐢 𝐥𝐮 𝐚𝐣𝐠🏃`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6283173142772@s.whatsapp.net"] // ganti nomer lu
			...
			switch(command) {
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`teksnya mana? contoh ${prefix}logowolf XNzul|Ganss`)
					reply(mess.wait)
           ...
           case 'demote':
              ...
              case 'owner':
               ...
               client.sendMessage(from, '𝐢𝐧𝐢 𝐩𝐚𝐜𝐚𝐫𝐤𝐮 𝐞𝐡𝐡, 𝐨𝐰𝐧𝐞𝐫𝐤𝐮 ><',MessageType.text, { quoted: mek} )
                break  
              ...
              case 'abangjago':
              case 'abgjago':
                    let eerrr = fs.readFileSync('./XNZUL/abangjago.mp3')
				...
				case 'tts':
					...
					const gtts = require('./XNZUL/gtts')(args[0])
